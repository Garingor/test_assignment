...
function takeUntilCancelAction(action$, prefix) {
  return operators.takeUntil(action$.pipe(operators.filter(function (action) {
...
function mapToLatest(action$, mapActionToObserable, prefix) {
  return operators.switchMap(function (_ref) {
    var action = _ref[0],
    ...
    return rxjs.concat(rxjs.of(action), mapActionToObserable(action, extraSideEffectConfig)).pipe(takeUntilCancelAction(action$, prefix));
  });
}

function takeEffectLatest(action$, state$, extraSideEffectObs$, mapActionToObserable, effectTypeArgs, prefix) {
  return action$.pipe(operators.withLatestFrom(extraSideEffectObs$), mapToLatest(action$, mapActionToObserable, prefix));
}
...
function takeEffectEvery(action$, state$, extraSideEffectObs$, mapActionToObserable, effectTypeArgs, prefix) {
  return action$.pipe(operators.withLatestFrom(extraSideEffectObs$), operators.mergeMap(function (_ref2) {
    var action = _ref2[0],
...
function actionToExhaustObservableEffect(action$, extraSideEffectObs$, mapActionToObserable, prefix) {
  return rxjs.merge(action$.pipe(operators.mergeMap(function (action) {
    if (action.type === prefix + CANCEL || action.type === prefix + CLEAN) {
      return rxjs.of(action);
    } else {
      return rxjs.empty();
    }
  })), action$.pipe(operators.withLatestFrom(extraSideEffectObs$), operators.exhaustMap(function (_ref3) {
    var action = _ref3[0],
...
function takeEffectGroupBy(action$, state$, extraSideEffectObs$, mapActionToObserable, effectTypeArgs, prefix) {
  ...
  return action$.pipe(operators.groupBy(groupByFn), operators.mergeMap(function ($group) {
    return $group.pipe(operators.withLatestFrom(extraSideEffectObs$), mapToLatest($group, mapActionToObserable, prefix));
  }));
}
...
var exportEffectCaller = rocketjumpCore.makeExportValue({
...
var addConfigToSideEffectDescritor = function addConfigToSideEffectDescritor(sideEffectDescriptor, config) {
  var newSideEffectDescriptor = _extends__default['default']({}, sideEffectDescriptor);
  ...
  if (config.takeEffect) {
    newSideEffectDescriptor.takeEffect = utils.arrayze(config.takeEffect);
  }
...
function run() {
  ...
  return rocketjumpCore.makeLibraryAction.apply(void 0, [RUN].concat(params));
...
function clean() {
  ...
  return rocketjumpCore.makeLibraryAction.apply(void 0, [CLEAN].concat(params));
...
function cancel() {
  ...
  return rocketjumpCore.makeLibraryAction.apply(void 0, [CANCEL].concat(params));
}
...
function reducer(prevState, action) {
  ...
  switch (type) {
    case PENDING:
      return _extends__default['default']({}, prevState, {
        error: null,
        pending: true
      });
    ...
    case CLEAN:
      ...
      return _extends__default['default']({}, prevState, defaultState);
...
function makeMutationReducer(mutation, name) {
  return function (state, action) {
    ...
    if (pieces[0] === MUTATION_PREFIX && pieces[1] === name) {
      ...
      return mutation.reducer(state, _extends__default['default']({}, action, {
        type: decoupleType
      }));
    }
...
function makeMutationsReducer(mutations) {
  var mutationsReducers = Object.keys(mutations).reduce(function (all, name) {
    ...
    return _extends__default['default']({}, all, (_extends2 = {}, _extends2[name] = makeMutationReducer(mutation, name), _extends2));
  }, {});
...
function handleOptRun(reducer, state, action) {
  ...
  return _extends__default['default']({}, nextState, {
    optimisticMutations: {
...
function createWithMutationsComputeState(computed, mutations) {
  ...
  var mutationsSelectors = computedKeys.filter(function (k) {
    ...
    return _extends__default['default']({}, selectors, (_extends2 = {}, _extends2[key] = function (state) {
      return utils.get(state, path);
    }, _extends2));
  }, {});
  return function computeState(state, selectors) {
    return computedKeys.reduce(function (computedState, selectorName) {
      ...
      if (mutationsSelectors[selectorName]) {
        ...
        return _extends__default['default']({}, computedState, (_extends3 = {}, _extends3[keyName] = mutationSelector(state.mutations), _extends3));
      }
      ...
      return _extends__default['default']({}, computedState, (_extends4 = {}, _extends4[keyName] = selector(state.root), _extends4));
    }, {});
  };
}
...
var makeRunTimeConfig = function makeRunTimeConfig(effectCaller, extraConfig) {
  ...
  var callEffect;
  callEffect = rocketjumpCore.squashExportValue(effectCaller, [placeholderEffectCaller].filter(Boolean)); // Use default effect caller
...
var ExtraSideEffectSubject = /*#__PURE__*/function (_BehaviorSubject) {
  _inheritsLoose__default['default'](ExtraSideEffectSubject, _BehaviorSubject);
  ...
  return function makeRxObservable(action$, state$, placeholderEffectCaller, prevObservable$ // <---- The observable to merge along
    ...
    function mapActionToObserable(action, _ref2) {
      ...
      if (!(isPromise(effectResult) || rxjs.isObservable(effectResult))) {
        return rxjs.throwError('The effect result is expect ' + ("to be a Promise or an RxObservable but '" + effectResult + "' ") + "was given. Please check your effect and effectCaller logic.");
      }
    ...
    var _arrayze = utils.arrayze(takeEffect),
    ...
    if (typeof effectType === 'function') {
      ...
      dispatchObservable = rxjs.merge(createEffect(action$.pipe(operators.filter(function (a) {
        return filterEffectActions(a, prefix);
      })), state$, extraSideEffectObs$, mapActionToObserable, effectTypeArgs, prefix), mergeObservable$.pipe(operators.filter(function (a) {
...
function makeActionCreator(name, mutation) {
  ...
  var actionCreator = function actionCreator() {
    ...
    return rocketjumpCore.makeLibraryAction.apply(void 0, [MUTATION_PREFIX + "/" + name + "/" + RUN].concat(params)).withMeta(meta);
  };
...
function enhanceActionCreators(mutations, actionCreators) {
  return Object.keys(mutations).reduce(function (enhancedActionCreators, name) {
    ...
    return _extends__default['default']({}, enhancedActionCreators, (_extends2 = {}, _extends2[name] = actionCreator, _extends2));
  }, actionCreators);
}
...
function makeMutationsSelectors() {
  return {
    ...
    getMutation: function getMutation(state, path) {
      return utils.get(state.mutations, path);
    }
  };
}
...
function enhanceMakeSelectors(mutations, makeSelectors) {
  ...
  return utils.kompose(makeSelectors, function (selectors) {
    return utils.proxyObject(selectors, makeMutationsSelectors);
  });
}
...
function makeMutationExport(mutation) {
  if (mutation.effectCaller) {
    return _extends__default['default']({}, mutation, {
...
function makeMutationsExport(mutations) {
  return Object.keys(mutations).reduce(function (mutationsExport, name) {
    ...
    return _extends__default['default']({}, mutationsExport, (_extends2 = {}, _extends2[name] = makeMutationExport(mutations[name]), _extends2));
  }, {});
}
...
function enhanceMakeExportWithMutations(rjConfig, extendExport) {
  ...
  if (rjConfig.mutations) {
    return _extends__default['default']({}, extendExport, {
...
function enhanceFinalExportWithMutations(rjObject, _ref) {
  ...
  var mutations = rjObject.mutations,
      rjEnhancedObject = _objectWithoutPropertiesLoose__default['default'](rjObject, ["mutations"]);
  ...
  if (Object.keys(reducersToCombine).length === 0) {
    ...
    withMutationsReducer = combineReducers(_extends__default['default']({
  ...
  return _extends__default['default']({}, rjEnhancedObject, {
...
var makeExport = (function (_, rjConfig, extendExport) {
  ...
  if (reducer$1) {
    reducer$1 = utils.proxyReducer(reducer$1, rjConfig.reducer);

    if (typeof rjConfig.composeReducer === 'function' || Array.isArray(rjConfig.composeReducer)) {
      var composeReducer = utils.arrayze(rjConfig.composeReducer);
      reducer$1 = utils.composeReducers.apply(void 0, [reducer$1].concat(composeReducer));
    }
  ...
  actionCreators = utils.proxyObject(actionCreators, rjConfig.actions); // Create the make selectors
  ...
  if (makeSelectors$1) {
    makeSelectors$1 = utils.kompose(makeSelectors$1, function (selectors) {
      return utils.proxyObject(selectors, rjConfig.selectors);
    });
  } // Default no computed
  ...
  if (rjConfig.computed) {
    ...
    computed = _extends__default['default']({}, computed, utils.invertKeys(rjConfig.computed));
  }

  var newExport = _extends__default['default']({}, extendExport, {
...
function shouldRocketJump(partialRjsOrConfigs) {
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(partialRjsOrConfigs), _step; !(_step = _iterator()).done;) {
    ...
    // Parital allowed
    if (rocketjumpCore.isPartialRj(partialRjOrConfig)) {
      continue;
    } // Rj Object not allowed


    if (rocketjumpCore.isObjectRj(partialRjOrConfig)) {
  ...
  if (!rocketjumpCore.isPartialRj(lastPartialOrConfig) && typeof lastPartialOrConfig === 'function') {
    return true;
  } // Not defined at last
...
function finalizeExport(mergegAlongExport, runConfig, finalConfig) {
  ...
  var sideEffect = mergegAlongExport.sideEffect,
      ...
      rjExport = _objectWithoutPropertiesLoose__default['default'](mergegAlongExport, ["sideEffect", "computed"]);

  var effectPipeline = sideEffect.effectPipeline,
      sideEffectConfig = _objectWithoutPropertiesLoose__default['default'](sideEffect, ["effectPipeline"]); // Create the make rx observable function using merged side effect descriptor!
  ...
  var computeState = rocketjumpCore.createComputeState(computed);

  var finalExport = _extends__default['default']({}, rjExport, {
...
var rj = rocketjumpCore.forgeRocketJump({
...
exports.RJ_CONFIG_PLACEHOLDER = RJ_CONFIG_PLACEHOLDER;
exports.RUN = RUN;
